...
export default function VentasClient({
  ...
  const total = carrito.reduce((sum, c) => sum + c.cantidad * c.producto.precio_venta, 0)

  async function handleRegistrar() {
    if (carrito.length === 0) return
    setLoading(true)
    try {
      const detalle = carrito.map((c) => ({
        producto_id: c.producto.id,
        cantidad: c.cantidad,
        precio_unitario: c.producto.precio_venta,
      }))
      await registrarVenta(detalle)
      setCarrito([])
      setSuccess(true)
    ...
    <div style={{ display: "flex", gap: "24px", height: "calc(100vh - 120px)" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "16px" }}>
            ...
            {resultados.map((p) => (
              ...
              >
                ...
                <div className="text-right">
                  ...
                  {p.stock_actual <= 0 && <p className="text-[11px] text-error">Sin stock</p>}
                </div>
              </button>
            ))}
          </div>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column" }} className="bg-surface-container-lowest rounded-xl border border-outline-variant shadow-sm p-md">
          ...
          {carrito.length === 0 ? (
            <p className="font-body-md text-body-md text-on-surface-variant flex-1 flex items-center justify-center">
            ...
            <div style={{ flex: 1, overflowY: "auto" }} className="space-y-xs">
              {carrito.map((c) => (
                <div key={c.producto.id} className="flex items-center justify-between py-xs px-sm rounded-lg hover:bg-surface-container-low">
                  <div style={{ flex: 1 }}>
                    <p className="font-body-md text-body-md font-semibold text-on-surface">{c.producto.nombre}</p>
        ...
        <div className="flex items-center justify-between p-md bg-surface-container-lowest rounded-xl border border-outline-variant shadow-sm">
          <div>
            ...
            <p className="font-headline-md text-headline-md text-secondary font-bold">S/ {total.toFixed(2)}</p>
          </div>
          <button
          ...
          >
            {loading ? "Registrando..." : success ? "✅ Venta registrada" : "Registrar Venta"}
          </button>
        </div>
      </div>
          ...
          {ventasHoy.map((v) => (
            <div key={v.id} className="p-sm rounded-lg border border-outline-variant/50 hover:bg-surface-container-low transition-colors">
              ...
              <p className="font-body-md text-body-md text-on-surface-variant text-[12px] mt-xs">
                {v.detalle_ventas?.map((d: any) => `${d.cantidad}x `).join(" • ")}
                {v.detalle_ventas?.length ?? 0} productos
              </p>
            </div>
          ))}
          {ventasHoy.length === 0 && (
            <p className="font-body-md text-body-md text-on-surface-variant text-center py-xl">No hay ventas hoy</p>
          )}
        </div>
      </div>
    </div>
