...
export default function VentasPage() {
  ...
  const historialContent = (
    <div style={{ display: "flex", flexDirection: "column", gap: "16px" }} className="bg-surface-container-lowest rounded-xl border border-outline-variant shadow-sm p-md">
      ...
      <div style={{ flex: 1, overflowY: "auto" }} className="space-y-xs">
        {ventasHoy.map((v) => (
          <div key={v.id} className="p-sm rounded-lg border border-outline-variant/50 hover:bg-surface-container-low transition-colors">
            ...
            <p className="text-[12px] text-on-surface-variant mt-xs">{v.detalle_ventas?.length ?? 0} productos</p>
          </div>
        ))}
  ...
  return (
    <>
      ...
      {/* Tabs — móvil */}
      <div className="md:hidden flex mb-4 bg-gray-100 rounded-xl p-1">
        <button onClick={() => setTab("carrito")} className={`flex-1 py-2 text-sm font-semibold rounded-lg transition-all ${tab === "carrito" ? "bg-white text-primary shadow-sm" : "text-gray-500"}`}>
          Nueva Venta
        </button>
        <button onClick={() => setTab("historial")} className={`flex-1 py-2 text-sm font-semibold rounded-lg transition-all ${tab === "historial" ? "bg-white text-primary shadow-sm" : "text-gray-500"}`}>
          Historial
        </button>
      </div>

      {/* Desktop: two columns */}
      <div className="hidden md:flex" style={{ gap: "24px", height: "calc(100vh - 120px)" }}>
      ...
      {/* Mobile: active tab */}
      <div className="md:hidden" style={{ height: "calc(100vh - 180px)", overflowY: "auto" }}>
        {tab === "carrito" ? carritoContent : historialContent}
      </div>
    </>
  )
}
